...
import { getAllSurahIdUnfiltered, getSurahNameAndAyat, getTafsir } from '../../data-sources/data-sources';
    ...
    <>
      ...
      <main style={{ width: '95%', marginLeft: 'auto', marginRight: 'auto' }} className='my-16'>
        <section className='bg-orange-200 p-4 mb-4 rounded-lg'>
          <p className='text-yellow-800 leading-loose'><strong>Catatan</strong>: Tafsir Quran Surat {name} berdasarkan sumber dari {source}</p>
        </section>
        <ul>
          {tafsirArray.map((ayat) => <AyatTafsirListItem key={ayat.ayatNumber} {...ayat} /> )}
        </ul>
      </main>
      {
        surahId
