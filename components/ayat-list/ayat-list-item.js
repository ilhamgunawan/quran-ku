...
import { storeAyat, removeAyat, getAyat } from '../../bookmark-utils/bookmark-utils';
  ...
  return (
    <li className='mb-5 border-b pb-3'>
      ...
      <div className='flex flex-col'>
        <div className='grid'>
          <p style={{ color: 'white' }} className='arabic-text col-start-1 h-10 w-10 mt-auto mb-6 text-2xl leading-none bg-blue-300 text-white flex justify-center items-center rounded-full'>{getArabicNumber(ayatNumber)}</p>
          <p lang='ar' className='arabic-text col-start-2 ml-auto mr-0 text-right'>{arabic}</p>
        </div>
        <div className='flex items-center'>
          ...
          }
          {
            bookmarkNotif.display
              ? <p className='text-xs text-yellow-800 bg-orange-200 p-1 rounded'>{bookmarkNotif.message}</p>
              : null
          }       
        </div>
        <p lang='id' className='mt-10 text-gray-700 leading-relaxed text-justify'>{translations}</p>
      </div>
    </li>
  );
};
